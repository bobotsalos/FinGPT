...
openai = OpenAI(
    api_key= "3RqOFKWEDsUpCBkPreg75ajkeA1IG3Fe",  #Replace with your actual DeepInfra api key
...
def get_sentiment(symbol, *texts):
    ...
    #        model='deepseek-ai/DeepSeek-R1-Distill-Llama-70B',
            model='deepseek-ai/DeepSeek-V3',  #  uses  DeepSeek V3 model
            messages=conversation,
            temperature=0, # 0 for deterministic output
            max_tokens=50, #  Limit the response length
            stream=stream, # Enable streaming if desired
        )
        ...
        if stream:
            ...
            content = chat_completion.choices[0].message.content # get the content of the response
            print(content)
...
def process_csv(input_csv_path, output_csv_path, batch_size=5, chunk_size=1000):
    ...
    for chunk_number, chunk in enumerate(chunks):
        ...
        chunk.columns = chunk.columns.str.capitalize() # Capitalize column names to match the expected format
        # Create empty sentiment_deepseek column if missing
        if model_used not in chunk.columns:
            chunk[model_used] = np.nan
        # Add the sentiment_deepseek column values
...
if __name__ == "__main__":
    input_file = './DATASETS/filtered_nasdaq_news.csv'  # Replace with your input file path
    output_file = './DATASETS/sentiment_deepseek_' + input_file
